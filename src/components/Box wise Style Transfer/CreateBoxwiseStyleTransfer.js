...
const CreateBoxwiseStyleTransfer = ({route}) => {
  ...
  const getData = async () => {
    ...
    let LISTAPIOBJ = await APIServiceCall.getBoxWiseStyleTransferCreateList(
      obj,
    );
    set_isLoading(false);
  ...
  const validateBarCode = async (id, fromLocationId) => {
    let userName = await AsyncStorage.getItem('userName');
    let userPsd = await AsyncStorage.getItem('userPsd');
    let usercompanyId = await AsyncStorage.getItem('companyId');
    let companyObj = await AsyncStorage.getItem('companyObj');
  ...
  const getDataFromBacode = async (id, fromLocationId) => {
    let userName = await AsyncStorage.getItem('userName');
    let userPsd = await AsyncStorage.getItem('userPsd');
    let usercompanyId = await AsyncStorage.getItem('companyId');
    let companyObj = await AsyncStorage.getItem('companyObj');
  ...
  const submitAction = async tempObj => {
    ...
    let SAVEAPIObj = await APIServiceCall.saveCreateBoxWiseStyleTransfer(
      tempObj,
    );
    set_isLoading(false);
...
export default CreateBoxwiseStyleTransfer;
