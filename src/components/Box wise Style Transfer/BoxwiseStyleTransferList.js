...
import BoxwiseStyleTransferListUI from './BoxwiseStyleTransferListUI';

const BoxwiseStyleTransferList = ({route}) => {
  ...
  const getInitialData = async (page = 0, reload = false) => {
    ...
    try {
      ...
      let LISTAPIOBJ = await APIServiceCall.LoadAllBoxWiseStyleTransferList(
        obj,
      );
      set_isLoading(false);
      ...
      if (LISTAPIOBJ && LISTAPIOBJ.statusData) {
        if (LISTAPIOBJ && LISTAPIOBJ.responseData) {
          // console.log('LISTAPIOBJ.responseData ==> ', LISTAPIOBJ.responseData.particulars);
          // set_itemsArray(prevItems =>
          //   reload
          //     ? LISTAPIOBJ.responseData.particulars
          //     : [...prevItems, ...LISTAPIOBJ.responseData.particulars],
          // );
          set_itemsArray(LISTAPIOBJ.responseData.particulars);
  ...
  const actionOnRow = (item, index) => {
    navigation.navigate('SaveBoxwiseStyleTransfer', {item: item});
  };
  ...
  const handleInandOutDcPdfs = async (transferId, type) => {
    let userName = await AsyncStorage.getItem('userName');
    let userPsd = await AsyncStorage.getItem('userPsd');
    let usercompanyId = await AsyncStorage.getItem('companyId');
    let companyObj = await AsyncStorage.getItem('companyObj');
    ...
    let obj = {
      ...
      compIds: usercompanyId,
      company: JSON.parse(companyObj),
      styleTransferId: transferId,
      typeOfPdf: type,
    };

    let apiUrl = await APIServiceCall.downloadBoxWieStyleTransferInOutDcPdfs();
   
    console.log('handleInandOutDcPdfs box style transfer req obj =====> ', obj);
  ...
  return (
    <BoxwiseStyleTransferListUI
      ...
      handleInandOutDcPdfs={handleInandOutDcPdfs}
    />
  );
};
export default BoxwiseStyleTransferList;
