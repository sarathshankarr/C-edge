...
import { View, StyleSheet, TouchableOpacity, Text, FlatList, ImageBackground, Image, TextInput } from 'react-native';
...
const StyleManageUI = ({ route, ...props }) => {
  ...
  const filterPets = (recName) => {
    ...
    if (isKeyboard.current === true) {
      ...
      const styleArray = nestedFilter.filter(style => (style.styleName.toUpperCase().includes(recName.toUpperCase()) || style.process.toUpperCase().includes(recName.toUpperCase()) || style.fabric.toUpperCase().includes(recName.toUpperCase())));
    ...
    <View style={[CommonStyles.mainComponentViewStyle]}>
      ...
      <View style={[CommonStyles.headerStyle]}>
        ...
        {filterArray && filterArray.length > 0 ? <View style={{flexDirection :'row'}}>
          ...
          <Text style={[CommonStyles.tylesHeaderTextStyle,{flex:1}]}>{'Process'}</Text>
