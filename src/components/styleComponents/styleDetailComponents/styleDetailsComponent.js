...
const StyleDetailsComponent = ({navigation, route, ...props}) => {
  ...
  const submitAction = async tempObj => {
    ...
    console.log(
      ...
      saveEditObj?.responseData,
      typeof saveEditObj?.responseData,
      saveEditObj?.responseData === true,
    );

    if (
      saveEditObj &&
      saveEditObj.statusData &&
      saveEditObj.responseData &&
      saveEditObj?.responseData !== 0
    ) {
      console.log('sucess');
      backBtnAction();
    } else {
  ...
  const saveEditStleBOMDetails = async tempObj => {
    set_isLoading(true);
    let userName = await AsyncStorage.getItem('userName');
    let userPsd = await AsyncStorage.getItem('userPsd');
    let usercompanyId = await AsyncStorage.getItem('companyId');
    ...
    (tempObj.menuId = 30),
      ...
      (tempObj.compIds = usercompanyId),
      (tempObj.userId = userId),
      console.log('saveEditStleBOMDetails save req ===> ', tempObj);
    tempObj.company = JSON.parse(companyObj);

    let saveEditObj = await APIServiceCall.saveEditStleBOMDetails(tempObj);
    set_isLoading(false);
    console.log(
      'response after approving saveEditStleBOMDetails ==>  ',
      saveEditObj?.responseData,
    );
    if (
      saveEditObj &&
      saveEditObj.statusData &&
      saveEditObj.responseData &&
      saveEditObj?.responseData === 'Trim details saved successfully'
    ) {
      console.log('sucess');
      backBtnAction();
    } else {
