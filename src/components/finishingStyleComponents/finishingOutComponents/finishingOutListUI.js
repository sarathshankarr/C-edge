...
const FinishingOutListUI = ({route, ...props }) => {
    ...
    return (
      <TouchableOpacity onPress={() => actionOnRow(item, index)} style={CommonStyles.cellBackViewStyle}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={CommonStyles.tylesTextStyle}>
            {item.totalQty}<Text style={[CommonStyles.tylesTextStyle,{flex:1.5,textAlign:'right',marginRight:wp('2%')}]}>{'/'+item.finishingOutQty}</Text>
            </Text>
          </View>
        </View>
      </TouchableOpacity> 
  ...
  export default FinishingOutListUI;