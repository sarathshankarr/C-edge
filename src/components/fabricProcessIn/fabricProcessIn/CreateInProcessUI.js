...
const CreateInProcessUI = ({route, navigation, ...props}) => {
  ...
  const handleSearchBatchNo = text => {
    ...
    if (text.trim().length > 0) {
      const filtered = batchNoList.filter(
        user =>
          user.name.toLowerCase().includes(text.toLowerCase()) ||
  ...
  const getBatchNoListByProcessId = async () => {
    ...
    let STOREDETAILSAPIObj = await APIServiceCall.getBatchNoListByProcessId(
      obj,
    );
    props.set_isLoading(false);

    if (STOREDETAILSAPIObj && STOREDETAILSAPIObj.statusData) {
      const processList = Object.keys(
        STOREDETAILSAPIObj.responseData.batchNoMap,
      ).map(key => ({
        id: key,
  ...
  const getBatchDetailsByBatchId = async () => {
    ...
    if (STOREDETAILSAPIObj && STOREDETAILSAPIObj?.statusData) {
      ...
      if (STOREDETAILSAPIObj?.responseData?.ordersMap) {
        const orderList = Object.keys(
          STOREDETAILSAPIObj?.responseData?.ordersMap,
        ).map(key => ({
          id: key,
          name: STOREDETAILSAPIObj.responseData.ordersMap[key],
        }));
        console.log(
          'after selecting batch id ==> ',
  ...
  const getBatchDetailsByBatchId2 = async () => {
    ...
    let STOREDETAILSAPIObj = await APIServiceCall.getBatchDetailsByBatchId2(
      obj,
    );
    props.set_isLoading(false);
  ...
  const getApiItemDetails = async () => {
    ...
    if (STOREDETAILSAPIObj && STOREDETAILSAPIObj.statusData) {
      if (STOREDETAILSAPIObj.responseData.myArrayList) {
        let result = STOREDETAILSAPIObj.responseData.myArrayList.map(
          item => item.map,
        );
  ...
  const getDesignNoListIfPrinting = async () => {
    ...
    let STOREDETAILSAPIObj = await APIServiceCall.getDesignNoListIfPrinting(
      obj,
    );
    props.set_isLoading(false);
    ...
    if (STOREDETAILSAPIObj && STOREDETAILSAPIObj.statusData) {
      if (STOREDETAILSAPIObj.responseData.designMap1) {
        if (STOREDETAILSAPIObj.responseData.designMap1) {
          const designList = Object.keys(
            STOREDETAILSAPIObj.responseData.designMap1,
          ).map(key => ({
            id: key,
  ...
  const getDesignNoListIfNotPrinting = async () => {
    ...
    let STOREDETAILSAPIObj = await APIServiceCall.getDesignNoListIfNotPrinting(
      obj,
    );
    props.set_isLoading(false);
    ...
    if (STOREDETAILSAPIObj && STOREDETAILSAPIObj.statusData) {
      if (STOREDETAILSAPIObj.responseData.designMap) {
        const designList = Object.keys(
          STOREDETAILSAPIObj.responseData.designMap,
        ).map(key => ({
          id: key,
  ...
  const getMatchingNoList = async () => {
    ...
    if (STOREDETAILSAPIObj && STOREDETAILSAPIObj.statusData) {
      if (STOREDETAILSAPIObj.responseData.matchingMap) {
        const matchingList = Object.keys(
          STOREDETAILSAPIObj.responseData.matchingMap,
        ).map(key => ({
          id: key,
  ...
  const getBatchListAfterPriniting = async mId => {
    ...
    let STOREDETAILSAPIObj = await APIServiceCall.getBatchListAfterPriniting(
      obj,
    );
    props.set_isLoading(false);
  ...
  const getfabIssuedafterPrinting = async mId => {
    ...
    let STOREDETAILSAPIObj = await APIServiceCall.getfabIssuedafterPrinting(
      obj,
    );
    props.set_isLoading(false);
    ...
    if (STOREDETAILSAPIObj && STOREDETAILSAPIObj.statusData) {
      if (STOREDETAILSAPIObj.responseData) {
        if (Number(processId) === 608 || Number(processId) === 609) {
          ...
          set_fabricIssuedLimit(
            STOREDETAILSAPIObj.responseData?.poft_printend_mtr,
          );
        ...
        <View
          ...
          <View
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                set_showBatchNoList(!showBatchNoList);
              }}>
              <View>
                <View style={[styles.SectionStyle1, {}]}>
                  <View style={{flexDirection: 'column'}}>
          ...
          {processId && (
            <View
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  set_showOrderNoList(!showOrderNoList);
                }}>
                <View>
                  <View style={[styles.SectionStyle1, {}]}>
                    <View style={{flexDirection: 'column'}}>
          ...
          {processId && (
            <View
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  set_showDesignNoList(!showDesignNoList);
                }}>
                <View>
                  <View style={[styles.SectionStyle1, {}]}>
                    <View style={{flexDirection: 'column'}}>
          ...
          {processId && processId !== '591' && (
            <View
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  set_showMatchingNoList(!showMatchingNoList);
                }}>
                <View>
                  <View style={[styles.SectionStyle1, {}]}>
                    <View style={{flexDirection: 'column'}}>
                    ...
                    <View key={index} style={styles.table_body_single_row}>
                      <View style={{width: '10%', alignItems: 'center'}}>
                        <RadioButton
                          value="Yes"
                          status={
                            selectedIndex === index ? 'checked' : 'unchecked'
                          }
                      ...
                      <View style={{width: '22%', alignItems: 'flex-end'}}>
                        <Text style={styles.table_data}>
                          {Number(item?.ordermtr)?.toFixed(2)}
                        </Text>
                      </View>
                      <View style={{width: '2%'}} />
                      <View style={{width: '22%', alignItems: 'flex-end'}}>
                        <Text style={styles.table_data}>
                          {Number(item?.totMtr)?.toFixed(2)}
                        </Text>
                      </View>
                      <View style={{width: '2%'}} />
                      <View style={{width: '22%', alignItems: 'flex-end'}}>
                        <Text style={styles.table_data}>
                          {Number(item?.remainTot)?.toFixed(2)}
                        </Text>
                      </View>
                    </View>
                  ))}
