...
import BillGenerationListUI from './BillGenerationListUI';

const BillGenerationList = ({route}) => {
  ...
  const getInitialData = async (page = 0, reload = false) => {
    ...
    try {
      ...
      let LISTAPIOBJ = await APIServiceCall.LoadAllBillGenerationList(obj);
      set_isLoading(false);
...
export default BillGenerationList;
