...
const CreateRequestUi = ({ route, ...props }) => {
    ...
    useEffect(() => {
        if (dataFromStock?.row && dataFromStock?.id) {
            console.log("Inside stock type use efect ===========> ", dataFromStock.row, dataFromStock.id)
    ...
    useEffect(() => {
        if (dataFromStockType?.row && dataFromStockType?.id) {
            console.log("Inside stock  use efect ===========> ", dataFromStockType.row, dataFromStockType.id)
    ...
    const getFabricByStyleId1 = async () => {
        ...
        if (STOREDETAILSAPIObj && STOREDETAILSAPIObj.statusData) {
            ...
            set_itemsObj((prev) => ({ ...prev, availFabricQty: STOREDETAILSAPIObj.responseData.availFabricQty }));
            set_itemsObj((prev) => ({ ...prev, uomfabric: STOREDETAILSAPIObj.responseData.uomType }));

            if (STOREDETAILSAPIObj?.responseData?.trimConstructionsList && STOREDETAILSAPIObj?.responseData?.trimConstructionsList?.length > 0) {
                setRows([]); // Clear previous rows
                STOREDETAILSAPIObj.responseData.trimConstructionsList.forEach((item, index) => {
                    setRows(prevRows => [
                        ...prevRows,
                        {
                            id: index,
            ...
            console.log("Values to Prepopulate", STOREDETAILSAPIObj.responseData.approvedFabric, STOREDETAILSAPIObj.responseData.approvedFabricConsumption, STOREDETAILSAPIObj.responseData.allowanceFabric, STOREDETAILSAPIObj.responseData.totalFabricApprovedWithAllowance, STOREDETAILSAPIObj.responseData.availFabricQty, STOREDETAILSAPIObj.responseData.uomType)
        } else {
    ...
    const getStockQty = async (rowId, trimId) => {
        ...
        if (STOREDETAILSAPIObj && STOREDETAILSAPIObj.statusData) {
            ...
            console.log("getStockQty==> ", STOREDETAILSAPIObj.responseData.uomstock, STOREDETAILSAPIObj.responseData.stockAvailQty)
        } else {
            // popUpAction(Constant.SERVICE_FAIL_MSG, Constant.DefaultAlert_MSG, 'OK', true, false);
    ...
    const getRmQtyByLot = async () => {
        ...
        if (STOREDETAILSAPIObj && STOREDETAILSAPIObj.statusData) {
            set_stockQtyItemsObj((prev) => ({ ...prev, uomstock: STOREDETAILSAPIObj.responseData.uomstock }));
            set_stockQtyItemsObj((prev) => ({ ...prev, stockAvailQty: STOREDETAILSAPIObj.responseData.stockAvailQty }));
        } else {
            popUpAction(Constant.SERVICE_FAIL_MSG, Constant.DefaultAlert_MSG, 'OK', true, false);
        }
    ...
    const actionOnStocks = async (item, rowId) => {
        ...
        setRows(rows.map(row => row.id === rowId
            ? { ...row, stock: item?.name, stockId: item?.id, showStocksList: false }
            : row
        ));
    ...
    const actionOnStockTypes = async (item, rowId) => {
        ...
        setRows(rows.map(row => row.id === rowId
            ? { ...row, stockType: item?.name, stockTypeId: item?.id, showStockTypesList: false }
            : row
        ));
    ...
    const handleSearchStockType = (text, rowId) => {
        setRows(rows.map(r =>
            r.id === rowId
                ? {
                    ...r, filteredStockTypes: props.lists.getStockTypes.filter(stockType =>
    ...
    const handleSearchStock = (text, rowId) => {
        setRows(rows.map(r =>
            r.id === rowId
                ? {
                    ...r, filteredStocks: r.stocksList.filter(stock =>
        ...
        <View style={[CommonStyles.mainComponentViewStyle]}>
            ...
            <KeyboardAwareScrollView enableOnAndroid={true} extraHeight={130} extraScrollHeight={130} showsVerticalScrollIndicator={false}>
                <View style={{ marginBottom: hp('5%') }}>
                    ...
                    <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: hp('3%'), backgroundColor: '#ffffff' }}>
                        ...
                        <View style={{ flexDirection: 'column', marginTop: hp('1%') }}>
                            ...
                            {generalRadio === "No" && (<View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: hp('1%') }}>
                            ...
                            {generalRadio === "No" && (<View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: hp('1%') }}>
                                ...
                                <Text style={{ marginRight: wp('2%'), color: '#000', color: '#000' }}>Yes</Text>
                                <RadioButton
                                    value="Yes"
                                    status={displayStyleRadio === 'Yes' ? 'checked' : 'unchecked'}
                                    onPress={() => set_displayStyleRadio('Yes')}
                                />
                                <Text style={{ marginRight: wp('2%'), marginLeft: wp('4%'), color: '#000' }}>No</Text>
                                <RadioButton
                                    value="No"
                                    status={displayStyleRadio === 'No' ? 'checked' : 'unchecked'}
                    ...
                    {generalRadio === 'No' && <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: hp('3%'), backgroundColor: '#ffffff' }} >

                        <TouchableOpacity style={{ flexDirection: 'row', borderWidth: 0.5, borderColor: "#D8D8D8", borderRadius: hp("0.5%"), width: wp("90%"), }} onPress={() => { set_showStylesList(!showStylesList) }}>

                            <View style={[styles.SectionStyle1, {}]}>

                                <View style={{ flexDirection: 'column', }}>
                                    <Text style={stylesId ? [styles.dropTextLightStyle] : [styles.dropTextInputStyle]}>{'Styles '}</Text>
                                    <Text style={[styles.dropTextInputStyle]}>{stylesId ? stylesName : 'Select Style'}</Text>
                                </View>
                    ...
                    <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: hp('2%'), backgroundColor: '#f8f8f8' }}>
                        <TouchableOpacity
                            ...
                            onPress={() => { set_showFabricList(!showFabricList) }}
                        >
                            <View style={[styles.SectionStyle1, {}]}>
                                <View style={{ flexDirection: 'column' }}>
                                    <Text style={fabricId ? [styles.dropTextLightStyle] : [styles.dropTextInputStyle]}>{'Fabric '}</Text>
                                    <Text style={[styles.dropTextInputStyle]}>{fabricId ? fabricName : 'Select fabric'}</Text>
                                </View>
                            </View>
                            <View style={{ justifyContent: 'center' }}>
                                <Image source={downArrowImg} style={styles.imageStyle} />
                    ...
                    {generalRadio === 'No' && <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: hp('1%') }} >
                        <TextInputComponent
                            inputText={itemsObj?.approvedFabric ? `${itemsObj?.approvedFabric} (${itemsObj?.approvedFabricConsumption})` : ''}
                    ...
                    {generalRadio === 'No' && <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: hp('1%') }} >
                        <TextInputComponent
                            inputText={itemsObj?.allowanceFabric ? itemsObj?.allowanceFabric : ''}
                    ...
                    {generalRadio === 'No' && <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: hp('1%') }} >
                        <TextInputComponent
                            inputText={itemsObj?.totalFabricApprovedWithAllowance ? itemsObj?.totalFabricApprovedWithAllowance?.toString() : ''}
                    ...
                    <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: hp('1%') }} >
                        <TextInputComponent
                            inputText={itemsObj?.availFabricQty ? itemsObj?.availFabricQty?.toString() : '0'}
                    ...
                    <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: hp('2%'), backgroundColor: '#ffffff' }} >
                        ...
                        <TouchableOpacity style={{ flexDirection: 'row', borderWidth: 0.5, borderColor: "#D8D8D8", borderRadius: hp("0.5%"), width: wp("90%"), }} onPress={() => { set_showProcessList(!showProcessList) }}>
                            ...
                            <View style={[styles.SectionStyle1, {}]}>
                                ...
                                <View style={{ flexDirection: 'column', }}>
                                    <Text style={processId ? [styles.dropTextLightStyle] : [styles.dropTextInputStyle]}>{'Process '}</Text>
                                    <Text style={[styles.dropTextInputStyle]}>{processId ? processName : 'Select Process'}</Text>
                                </View>
                            ...
                            <View style={styles.popSearchViewStyle}>
                                <ScrollView nestedScrollEnabled={true}>
                                    {props?.lists?.getMenus?.map((item) => (
                                        <TouchableOpacity key={item?.id} onPress={() => actionOnProcess(item.id, item.name)}>
                                            <View style={styles.flatview}>
                                                <Text style={styles.dropTextInputStyle}>{item?.name}</Text>
                                            </View>
                                        </TouchableOpacity>
                    ...
                    <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: hp('2%'), backgroundColor: '#ffffff' }}>
                        <TouchableOpacity
                            style={{ flexDirection: 'row', borderWidth: 0.5, borderColor: "#D8D8D8", borderRadius: hp("0.5%"), width: wp("90%") }}
                            onPress={() => { set_showUnitMasterList(!showUnitMasterList) }}
                        >
                            <View style={[styles.SectionStyle1, {}]}>
                                <View style={{ flexDirection: 'column' }}>
                                    <Text style={unitMasterId ? [styles.dropTextLightStyle] : [styles.dropTextInputStyle]}>{'Unit Master '}</Text>
                                    <Text style={[styles.dropTextInputStyle]}>{unitMasterId ? unitMasterName : 'Select Unit Master'}</Text>
                                </View>
                            </View>
                            <View style={{ justifyContent: 'center' }}>
                                <Image source={downArrowImg} style={styles.imageStyle} />
                    ...
                    <View style={{ width: '90%', marginTop: 20, marginBottom: 20, marginHorizontal: 15 }}>
                    ...
                    {displayStyleRadio === "No"  && <View style={{ width: '90%', marginTop: 20, marginBottom: 30, marginHorizontal: 15 }}>
                        <TouchableOpacity onPress={addRow}
                            ...
                            <Text style={{ color: '#fff', fontSize: 16, fontWeight: 'bold' }}>Add Stock</Text>
                        </TouchableOpacity>
                    </View>}
                        ...
                        <ScrollView nestedScrollEnabled={true} horizontal>
                            <View style={styles.table}>
                                ...
                                <View style={styles.table_head}>
                                    <View style={{ width: 60 }}><Text style={styles.table_head_captions}>Action</Text></View>
                                    <View style={{ width: 200 }}><Text style={styles.table_head_captions}>Stock Type</Text></View>
                                    <View style={{ width: 200 }}><Text style={styles.table_head_captions}>Stock</Text></View>
                                    <View style={{ width: 80 }}><Text style={styles.table_head_captions}>Size</Text></View>
                                    <View style={{ width: 100 }}><Text style={styles.table_head_captions}>Available Qty</Text></View>
                                    <View style={{ width: 100 }}><Text style={styles.table_head_captions}>Input Quantity</Text></View>
                                    <View style={{ width: 100 }}><Text style={styles.table_head_captions}>Stock UOM</Text></View>
                                </View>
                                ...
                                {rows.map((row) => (
                                    <View key={row.id} style={styles.table_body_single_row}>
                                        <View style={{ width: 60 }}>
                                            <TouchableOpacity style={{ alignItems: 'center' }} onPress={() => RemoveRow(row.id)}>
                                        ...
                                        <View style={{ width: 200 }}>
                                            <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: hp('1%'), backgroundColor: row?.editStockType ? '#ffffff' : '#dedede' }}>
                                                <TouchableOpacity
                                                    style={{
                                                        flexDirection: 'row',
                                                        borderWidth: 0.5,
                                                    ...
                                                    onPress={() => {
                                                        setRows(rows.map(r =>
                                                            r.id === row.id
                                                                ? { ...r, showStockTypesList: !r.showStockTypesList, showStocksList: false }
                                                ...
                                                >
                                                    <View style={[styles.SectionStyle1]}>
                                                        <View style={{ flexDirection: 'column' }}>
                                                            <Text style={row.stockType ? styles.dropTextLightStyle : styles.dropTextInputStyle}>
                                                                {'StockTypes '}
                                                            </Text>
                                                            <Text style={styles.dropTextInputStyle}>
                                                                {row.stockTypeId ? row.stockType : 'Select Stock Type'}
                                                            </Text>
                                                        </View>
                                                    </View>
                                                    <View style={{ justifyContent: 'center' }}>
                                                        <Image source={downArrowImg} style={{ height: 40, width: 40 }} />
                                                    </View>
                                                </TouchableOpacity>
                                                ...
                                                {row.showStockTypesList && row.editStockType && (
                                                    <View style={styles.popSearchViewStyle}>
                                                        <TextInput
                                                            ...
                                                            onChangeText={(text) => handleSearchStockType(text, row.id)}
                                                            placeholderTextColor="#000"
                                                        />
                                                        <ScrollView nestedScrollEnabled={true}>
                                                            {row.filteredStockTypes.length === 0 ? (
                                                                ...
                                                                row.filteredStockTypes.map((item) => (
                                                                    <TouchableOpacity key={item?.id} onPress={() => actionOnStockTypes(item, row.id)}>
                                                                        <View style={styles.flatview}>
                                                                            <Text style={styles.dropTextInputStyle}>{item?.name}</Text>
                                                                        </View>
                                                                    </TouchableOpacity>
                                        ...
                                        {/* Stock Dropdown */}
                                        <View style={{ width: 200 }}>
                                            <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: hp('2%'), backgroundColor: row.editStock ? '#ffffff' : '#dedede'}}>
                                                <TouchableOpacity
                                                    style={{
                                                        flexDirection: 'row',
                                                        borderWidth: 0.5,
                                                    ...
                                                    onPress={() => {
                                                        setRows(rows.map(r =>
                                                            r.id === row.id
                                                                ? { ...r, showStocksList: !r.showStocksList, showStockTypesList: false }
                                                ...
                                                >
                                                    <View style={[styles.SectionStyle1]}>
                                                        <View style={{ flexDirection: 'column' }}>
                                                            <Text style={row.stockId ? styles.dropTextLightStyle : styles.dropTextInputStyle}>
                                                                {'Stock '}
                                                            </Text>
                                                            <Text style={styles.dropTextInputStyle}>
                                                ...
                                                {row.showStocksList && row.editStock &&  (
                                                    <View style={styles.popSearchViewStyle}>
                                                        <TextInput
                                                            ...
                                                            onChangeText={(text) => handleSearchStock(text, row.id)}
                                                            placeholderTextColor="#000"
                                                        />
                                                        <ScrollView nestedScrollEnabled={true}>
                                                            {row.filteredStocks.length === 0 ? (
                                                                ...
                                                                row.filteredStocks.map((item) => (
                                                                    <TouchableOpacity key={item?.id} onPress={() => actionOnStocks(item, row.id)}>
                                                                        <View style={styles.flatview}>
                                                                            <Text style={styles.dropTextInputStyle}>{item?.name}</Text>
                                                                        </View>
                                                                    </TouchableOpacity>
                                        ...
                                        <View style={{ width: 100 }}>
                                            <TextInput
                                                ...
                                                value={row.inputQty}
                                                onChangeText={(text) => {
                                                    setRows(rows.map(r => r.id === row.id ? { ...r, inputQty: text } : r));
                                                }}
                                            />
                                        </View>
                                        <View style={{ width: 100 }}>
