...
const GoodsReceiptNoteListUI = ({route, navigation, ...props}) => {
    ...
    const styleArray = ItemsArray.filter(item => {
      return (
        (item?.poNumberWithPrefix !== '' &&
          item?.poNumberWithPrefix?.toLowerCase().includes(searchTerm)) ||
        (item?.vendorName !== '' &&
          item?.vendorName?.toLowerCase().includes(searchTerm)) ||
        (item?.rmFabric !== '' &&
          item?.rmFabric?.toLowerCase().includes(searchTerm)) 
      )
    });
...
export default GoodsReceiptNoteListUI;
