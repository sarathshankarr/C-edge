...
import BoxPackingListUI from './BoxPackingListUI';

const BoxPackingList = ({route}) => {
  ...
  const getInitialData = async (page = 0, reload = false) => {
    ...
    try {
      ...
      let LISTAPIOBJ = await APIServiceCall.LoadAllBoxPacking(obj);
      set_isLoading(false);
