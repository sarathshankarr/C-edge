...
const RawMaterialTypeListUI = ({route, navigation, ...props}) => {
  ...
  const filterPets = name => {
    ...
    const styleArray = ItemsArray.filter(item => {
      return (
        (item?.trimtype !== '' &&
          item?.trimtype?.toLowerCase().includes(searchTerm)) ||
        (item?.trim_type_code !== '' &&
          item?.trim_type_code?.toLowerCase().includes(searchTerm)) 
      );
    });
  ...
  return (
    <View style={[CommonStyles.mainComponentViewStyle]}>
      ...
      <AddNewItem navItem={'CreateRawMaterialTypeComponent'} />
...
export default RawMaterialTypeListUI;
