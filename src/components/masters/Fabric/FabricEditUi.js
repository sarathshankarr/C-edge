...
const FabricEditUi = ({route, ...props}) => {
  ...
  const [trimFabricRadio, set_trimFabricRadio] = useState('');
  const [updateRollWiseQtyRadio, set_updateRollWiseQtyRadio] = useState('');

  const [addBrandProjectName, setAddBrandProjectName] = useState('');
  const [addBrandProjectDescription, setAddBrandProjectDescription] =
    useState('');
  ...
  useEffect(() => {
    if (props.itemsObj) {
      ...
      if(props.itemsObj.fabrictypeMap){
        const fabrictypeMapList = Object.keys(props.itemsObj.fabrictypeMap).map(key => ({
          id: key,
          name: props.itemsObj.fabrictypeMap[key]
        }));
        ...
        setFabricTypeList(updatedfabrictypeMapList);
        set_filteredFabricType(updatedfabrictypeMapList);
      }

      if(props.itemsObj.brandsMap){
        const brandsMapList = Object.keys(props.itemsObj.brandsMap).map(key => ({
          id: key,
        ...
        setBrandOrProjectList(updatedbrandsMapList);
        setFilteredBrandOrProject(updatedbrandsMapList);
      }

      if (props.itemsObj.fabricBrandId) {
        setBrandOrProjectId(props.itemsObj.fabricBrandId);
        setBrandOrProjectName(
          props.itemsObj.brandsMap[props.itemsObj.fabricBrandId],
        );
      }
      if (props.itemsObj.colorid) {
        set_colorId(props.itemsObj.colorid);
        set_colorName(props.itemsObj.colorMap[props.itemsObj.colorid]);
      }
      if (props.itemsObj.locationId) {
        set_locationId(props.itemsObj.locationId);
        set_locationName(
          props.itemsObj.locationsMap[props.itemsObj.locationId],
        );
      }
      if (props.itemsObj.fabricTypeId) {
        set_fabricTypeId(props.itemsObj.fabricTypeId);
        set_fabricTypeName(
          props.itemsObj.fabrictypeMap[props.itemsObj.fabricTypeId],
        );
      }
      if (props.itemsObj.uomId) {
        set_uomId(props.itemsObj.uomId);
        set_uomName(props.itemsObj.uomMap[props.itemsObj.uomId]);
      }
      if (props.itemsObj.inv_limit) {
        setInventoryLimit(props.itemsObj.inv_limit);
      }
      if (props.itemsObj.fabricNo) {
        setFabricNo(props.itemsObj.fabricNo);
      }
      ...
      if (props.itemsObj.rate) {
        setRate(props.itemsObj.rate);
      }
      if (props.itemsObj.fabriccode) {
        setFabricCode(props?.itemsObj?.fabriccode);
      }
      if (props.itemsObj.fabricWidth) {
        setFabricWidth(props.itemsObj.fabricWidth);
      }
      set_updateRollWiseQtyRadio(
        props?.itemsObj?.aisbinSts === 1 ? 'Yes' : 'No',
      );

      if (props.itemsObj.bomststatus) {
        set_trimFabricRadio(props.itemsObj.bomststatus === 1 ? 'Yes' : 'No');
      }
    }
    console.log('edit details ==> ', props?.itemsObj);
  }, [props.itemsObj]);
  ...
  const submitAction = async () => {
    ...
    let tempObj = {
      fabricNo: fabricNo,
      designCodes: '',
      colorName: props.itemsObj.colorName,
      ...
      aisbinSts: props.itemsObj.aisbinSts,
      bomststatus: props.itemsObj.bomststatus,
      fabricBrandId: brandOrProjectId,
    };
        ...
        <View
          ...
          <View style={{marginTop: hp('2%')}}>
            <TextInput
              label="Fabric No *"
              value={fabricNo}
              editable={fabricNo ? false : true}
              mode="outlined"
              style={{backgroundColor: fabricNo ? '#E8E8E8' : '#fff'}}
          ...
          <View
            ...
            <RadioGroup
              ...
              onPress={() => console.log('handletrimFabricRadioChange')}
              layout="row"
              selectedId={
          ...
          <View
            ...
            <RadioGroup
              ...
              onPress={() => console.log('handleUpdateRollWiseQtyRadioChange')}
              layout="row"
              selectedId={
